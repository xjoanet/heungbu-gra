export const LANG = {
  ko: {
    ...
    slot_draw: '▶ 명대사 뽑기',
    ins_t: '설치는 3초', ins_d: 'MCP 서버로 원격 URL 한 줄만 붙이면 끝입니다. 이것저것 설치할 게 없어요.',
    guide_t: '설치하고 나서, 어떻게 하나요?',
    guide1: '위 URL을 복사해서 MCP 원격/HTTP 서버로 등록하세요. (클로드·커서·Hermes 등 어디든)',
    guide2: 'URL을 원격 서버로 등록하면, 바로 이 채팅 어디에서든 "ㅊㅊ"가 칭찬으로 발동됩니다. 프롬프트나 별도 설정이 필요 없어요.',
    guide2l: '참고 — 원격 URL (복사해서 서버 등록)',
    guide2c: 'https://mcp.heungbu26.com/mcp',
    guide3: '로컬에서 직접 돌리고 싶다면 npx 방식도 그대로 쓸 수 있어요.',
    guide3t: '등록 방법: MCP 클라이언트 설정(예: Claude Desktop의 claude_desktop_config.json, Claude Code는 claude mcp add 명령)의 mcpServers에 "command": "npx", "args": ["-y", "heungbu26-mcp"] 항목을 추가하면 됩니다. 매번 붙여넣지 않아도 모든 채팅에서 자동으로 작동해요.',
  ...
  en: {
    ...
    ins_t: '3-second install', ins_d: 'Add it as a remote/HTTP MCP server — that\'s it, nothing to install.',
    guide_t: 'OK, registered — now what?',
    guide1: 'Copy the URL above and add it as a remote/HTTP MCP server (Claude, Cursor, Hermes, etc).',
    guide2: 'Register the URL as a remote server and "ㅊㅊ" fires praise anywhere in this chat. No prompt, no extra setup.',
    guide2l: 'Note — remote URL (copy to register)',
    guide2c: 'https://mcp.heungbu26.com/mcp',
    guide3: 'Want to run it locally on your own machine? The npx route still works.',
    guide3t: 'How: in your MCP client config (e.g. claude_desktop_config.json for Claude Desktop, or the claude mcp add command for Claude Code), add under mcpServers "command": "npx", "args": ["-y", "heungbu26-mcp"]. It then runs in every chat automatically.',
